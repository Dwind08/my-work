...
public class SM2Util {
    ...
    public static boolean sm2WithSm3VerifyANS1(String publicKey, byte[] msg, byte[] signature) {
        byte[] rsSign = decodeDERSignature(signature);
        ECPublicKeyParameters ecPublicKeyParameters = encodePublicKey(Hex.decode(publicKey));
        SM2Signer signer = new SM2Signer();
        ParametersWithID parameters = new ParametersWithID(ecPublicKeyParameters, "1234567812345678".getBytes());
        signer.init(false, parameters);
        signer.update(msg, 0, msg.length);
        return signer.verifySignature(encodeDERSignature(rsSign));
    }
...
//            signature = decodeDERSignature(signer.generateSignature());
            signature = encodeDERSignature(decodeDERSignature(signer.generateSignature()));
        } catch (Exception e) {
